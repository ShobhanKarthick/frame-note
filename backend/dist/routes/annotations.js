...
router.get('/video/:videoId', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT a.*, u.name as author_name 
       FROM annotations a 
       JOIN users u ON a.user_id = u.id 
       WHERE a.video_id = $1 
       ORDER BY a.start_time ASC`, [videoId]);
...
router.post('/', async (req, res) => {
    try {
        ...
        const id = uuidv4();
        const result = await pool.query(`INSERT INTO annotations (id, video_id, user_id, start_time, end_time, text, type, drawing_data, attachments)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING *`, [id, video_id, user_id, start_time, end_time, text || '', type, drawing_data || null, JSON.stringify(attachments || [])]);
        // Fetch with author info
        const fullResult = await pool.query(`SELECT a.*, u.name as author_name 
       FROM annotations a 
       JOIN users u ON a.user_id = u.id 
       WHERE a.id = $1`, [id]);
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query('DELETE FROM annotations WHERE id = $1 RETURNING id', [id]);
...
router.post('/import', async (req, res) => {
    try {
        ...
        for (const ann of annotations) {
            const id = uuidv4();
            await pool.query(`INSERT INTO annotations (id, video_id, user_id, start_time, end_time, text, type, drawing_data, attachments)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
                id,
                videoHash,
...
router.delete('/video/:videoId', async (req, res) => {
    try {
        const { videoId } = req.params;
        const result = await pool.query('DELETE FROM annotations WHERE video_id = $1', [videoId]);
